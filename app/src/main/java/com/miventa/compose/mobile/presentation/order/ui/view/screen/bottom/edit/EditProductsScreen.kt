...
fun EditProductsScreen(editProductsInteractions: EditProductsInteractions) {
    ...
    Scaffold(
        topBar = {
            TopAppBar(title = { Text(text = stringResource(id = R.string.edit)) })
        },
        floatingActionButton = {
            FloatingActionButton(
                onClick = {
                    editProductsInteractions.navigateToCreateProduct()
                },
            ) {
                Icon(
...
private fun Context.handleEditProductsEvent(event: OrderUiEvent) {
    when (event) {
